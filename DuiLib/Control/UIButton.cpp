...
namespace DuiLib
{
   ...
   */
   ...
   {
      if ( _tcscmp ( pstrName, _T ( "normalimage" ) ) == 0 ) SetNormalImage ( pstrValue );             //常规
      else if ( _tcscmp ( pstrName, _T ( "hotimage" ) ) == 0 ) SetHotImage ( pstrValue );              //鼠标悬浮状态图片
      else if ( _tcscmp ( pstrName, _T ( "pushedimage" ) ) == 0 ) SetPushedImage ( pstrValue );        //鼠标按下的状态图片
      else if ( _tcscmp ( pstrName, _T ( "focusedimage" ) ) == 0 ) SetFocusedImage ( pstrValue );      //获得焦点时的状态图片
      else if ( _tcscmp ( pstrName, _T ( "disabledimage" ) ) == 0 ) SetDisabledImage ( pstrValue );    //禁用的状态图片
      else if ( _tcscmp ( pstrName, _T ( "foreimage" ) ) == 0 ) SetForeImage ( pstrValue );            //前景图片
      else if ( _tcscmp ( pstrName, _T ( "hotforeimage" ) ) == 0 ) SetHotForeImage ( pstrValue );      //鼠标悬浮 前景
   ...
   bool CButtonUI::SetButtonImage ( LPCTSTR file_name, unsigned int count /*= 4*/, bool horizon/*=true */ )
   {
      this->SetTag ( 12345 );
      ...
      for ( int i = 0; i < count; ++i )
      {
         ...
         TCHAR image_buff[256];
         _sntprintf ( image_buff, 256, _T ( "file='%s' source='%d,%d,%d,%d'" ),
            file_name, x_start, y_start, x_start + pic_width, y_start + pic_height
            );

         auto di_info = draw_info_array[i];
         if ( di_info->sDrawString != image_buff || NULL == di_info->pImageInfo )
         {
            di_info->Clear ( );
            di_info->sDrawString = image_buff;
         }
      }
      Invalidate ( );
      return true;
